...
InverseGammaWithSoftplusConcentrationRate <- ed$models$InverseGammaWithSoftplusConcentrationRate
...
In our example, since $D_y=1$, we have a univariate distribution $y \sim N( \hat{y}(x),\sigma_\epsilon^2 )$, where scalar $\sigma_\epsilon^2=\mathbf{\Sigma}_\epsilon$.
...
sigma <- InverseGammaWithSoftplusConcentrationRate(concentration=10*tf$ones( array(1L,1L) ),rate=tf$ones( array(1L,1L) ))
...
(Above, $\text{g}(u)$ is the "softplus" function described at the link below.)
...
qsigma = InverseGammaWithSoftplusConcentrationRate( concentration=10*tf$Variable(tf$random_normal(array(1L,1L))), rate=tf$Variable(tf$random_normal(array(1L,1L))))
```
...
Now, run variational inference with the [Kullback-Leibler divergence](https://en.wikipedia.org/wiki/Kullback%E2%80%93Leibler_divergence) in order to infer the model's latent variables given data. We specify $2000$ iterations.
